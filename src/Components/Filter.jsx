...
const Filter = ({ onSelectService }) => {
  ...
  return (
    <div className='flex flex-col items-center w-full mb-[100px]'>
      ...
      <div className='absolute bg-gray-200 h-[60px] w-[240px] rounded-[100px] mt-[200px]' />
      <div className='relative flex flex-wrap justify-center gap-4 py-3 mt-[50px]  rounded-[50px]  mx-[25px] '>
        
        {ServiceData.map((service) => (
          <button
            key={service.id}
            className={`w-[230px] h-12 rounded-[30px] border-none px-2  mb-2 hover:bg-white ${activeButton === service.id ? 'bg-white' : 'bg-gray-200'}`}
